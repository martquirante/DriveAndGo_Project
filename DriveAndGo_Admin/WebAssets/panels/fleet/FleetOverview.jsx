...
const VehicleImageComponent = ({ src, alt, className, iconSize = 48 }) => {
    ...
    if (!src || hasError) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center gap-1.5 p-3 bg-gradient-to-br from-slate-100 via-slate-200/80 to-slate-100 dark:from-[#0B1120] dark:via-[#111827] dark:to-[#0F172A] text-slate-400 dark:text-slate-500 transition-colors select-none">
                <IconCar sz={iconSize} c="text-slate-400 dark:text-slate-500 stroke-[1.5]" />
                <span className="text-[9.5px] font-bold tracking-wider uppercase text-slate-400 dark:text-slate-600">No Image Uploaded</span>
            </div>
        );
    }
...
const StatusCombobox = ({ value, onChange }) => {
    ...
    const options = [
        ...
        { key: 'low_rfid', label: '⚠️ Low RFID (< ₱200)' },
        { key: 'good_rfid', label: '🟢 Active RFID (≥ ₱200)' },
    ];
    ...
    return (
        <div className="relative shrink-0" ref={containerRef}>
            ...
            >
                <span className={`w-2 h-2 rounded-full ${value === 'available' ? 'bg-emerald-400 animate-pulse' : value === 'rented' ? 'bg-amber-400 animate-pulse' : value === 'maintenance' ? 'bg-purple-400' : value === 'low_rfid' ? 'bg-red-500 animate-pulse' : value === 'good_rfid' ? 'bg-blue-400' : 'bg-orange-500'}`}></span>
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-52 rounded-2xl bg-[var(--bg-card)] border border-orange-500/40 shadow-[0_12px_32px_rgba(0,0,0,0.6)] py-1.5 z-50 backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150">
                    ...
                    {options.map(opt => (
                        ...
                        >
                            <span className="flex items-center gap-2">
                                <span className={`w-2 h-2 rounded-full ${opt.key === 'available' ? 'bg-emerald-400' : opt.key === 'rented' ? 'bg-amber-400' : opt.key === 'maintenance' ? 'bg-purple-400' : opt.key === 'low_rfid' ? 'bg-red-500' : opt.key === 'good_rfid' ? 'bg-blue-400' : 'bg-orange-500'}`}></span>
                                {opt.label}
                            </span>
...
const VehicleCard = ({ v, onClick, onMapFocus }) => {
    ...
    return (
        ...
        >
            ...
            <div className="h-48 w-full relative overflow-hidden shrink-0 border-b border-[var(--border-color)] group-hover:border-orange-500/30 transition-colors bg-slate-100 dark:bg-[#0B1120] flex items-center justify-center">
...
const VehicleDrawer = ({ v, onClose, onRefresh, onEdit, onDelete, onShowQr, onPreviewDoc }) => {
    if (!v) return null;
    const st = getStatusStyle(v.status);
    const [rfidBal, setRfidBal] = useState(parseFloat(v.expressway_rfid_balance ?? v.expresswayRfidBalance ?? v.rfid_balance_autosweep ?? v.rfidBalancePHP ?? 500));
    ...
    // Extract all unique vehicle photos
    const allPhotos = useMemo(() => {
        let list = [];
        if (v.image) list.push(v.image);
        if (Array.isArray(v.photo_urls) && v.photo_urls.length > 0) list.push(...v.photo_urls);
        else if (Array.isArray(v.photoUrls) && v.photoUrls.length > 0) list.push(...v.photoUrls);
        else if (Array.isArray(v.photos) && v.photos.length > 0) list.push(...v.photos);
        else if (typeof v.photo_urls === 'string' && v.photo_urls.startsWith('[')) {
            try { list.push(...JSON.parse(v.photo_urls)); } catch(e){}
        }
        const unique = Array.from(new Set(list.filter(Boolean).map(resolveVehicleImgUrl)));
        return unique.length > 0 ? unique : (v.image ? [resolveVehicleImgUrl(v.image)] : []);
    }, [v]);

    // Auto-switch photos every 5 seconds if multiple exist
    useEffect(() => {
        if (allPhotos.length <= 1) return;
        const timer = setInterval(() => {
            setPhotoIdx(prev => (prev + 1) % allPhotos.length);
        }, 5000);
        return () => clearInterval(timer);
    }, [allPhotos.length]);

    useEffect(() => {
        const bal = parseFloat(v.expressway_rfid_balance ?? v.expresswayRfidBalance ?? v.ExpresswayRfidBalance ?? v.rfid_balance_autosweep ?? v.rfidBalanceAutosweep ?? v.rfidBalancePHP ?? 500);
        setRfidBal(bal);
    }, [v, v.expressway_rfid_balance, v.expresswayRfidBalance, v.rfidBalancePHP]);

    const handleRfidTopup = async (amountToAdd) => {
        const amt = parseFloat(String(amountToAdd).replace(/,/g, ''));
        if (isNaN(amt) || amt <= 0) return;
        setTopupLoading(true);
        setTopupSuccess('');
        try {
            const vehicleId = v.vehicle_id || v.id || v.VehicleId;
            const res = await fetch(`${API}/api/vehicles/${vehicleId}/rfid/topup`, {
                method: 'POST',
                headers: HEADERS,
            ...
            if (res.ok) {
                ...
                const updated = parseFloat(data.expresswayRfidBalance ?? data.ExpresswayRfidBalance ?? (rfidBal + amt));
                setRfidBal(updated);
                v.expressway_rfid_balance = updated;
                v.expresswayRfidBalance = updated;
                v.rfidBalancePHP = updated;
                setTopupSuccess(`+₱${amt.toLocaleString()} loaded!`);
                setTimeout(() => setTopupSuccess(''), 3500);
                if (onRefresh) onRefresh();
            } else {
                const updated = rfidBal + amt;
                setRfidBal(updated);
                v.expressway_rfid_balance = updated;
                v.expresswayRfidBalance = updated;
                v.rfidBalancePHP = updated;
            ...
            console.error('RFID topup error:', e);
            const updated = rfidBal + amt;
            setRfidBal(updated);
            v.expressway_rfid_balance = updated;
            v.expresswayRfidBalance = updated;
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[9999] flex items-center justify-center p-4 sm:p-6 animate-fade-in" onClick={onClose}>
            <div className="w-full max-w-lg md:max-w-xl max-h-[85vh] bg-[var(--bg-secondary)] backdrop-blur-2xl border border-[var(--border-color)] text-[var(--text-primary)] rounded-3xl shadow-[0_25px_60px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden relative animate-scale-up" onClick={e => e.stopPropagation()}>

                {/* Header Image & Vehicle Details Banner (with Fullscreen Lightbox & 5s Auto Carousel) */}
                <div 
                    className="h-52 relative shrink-0 overflow-hidden bg-slate-950 group cursor-pointer select-none"
                    onClick={() => setIsFullscreenImg(true)}
                    title="Click to view full screen gallery"
                >
                    {allPhotos.length > 0 ? (
                        <img 
                            key={allPhotos[photoIdx % allPhotos.length]} 
                    ...
                    <div className="absolute top-4 left-6 opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 text-white text-[10px] font-bold px-2.5 py-1 rounded-full backdrop-blur border border-white/20 flex items-center gap-1 z-10">
                        <IconMaximize sz={11} /> Click for Fullscreen
                    </div>

                    {/* Carousel Switching Controls (if multiple images) */}
                    {allPhotos.length > 1 && (
                        <div className="absolute top-4 right-16 flex items-center gap-1.5 z-10" onClick={e => e.stopPropagation()}>
                            <button 
                                onClick={(e) => { e.stopPropagation(); setPhotoIdx(prev => (prev - 1 + allPhotos.length) % allPhotos.length); }} 
                                className="p-1 rounded-full bg-black/60 hover:bg-orange-500 text-white border border-white/20 backdrop-blur transition-all active:scale-95 cursor-pointer"
                                title="Previous photo"
                            >
                                <IconChevronLeft sz={12} />
                            </button>
                            <span className="text-[10px] font-black text-white bg-black/60 px-2 py-0.5 rounded-full border border-white/20 backdrop-blur">
                                {photoIdx + 1}/{allPhotos.length}
                            </span>
                            <button 
                                onClick={(e) => { e.stopPropagation(); setPhotoIdx(prev => (prev + 1) % allPhotos.length); }} 
                                className="p-1 rounded-full bg-black/60 hover:bg-orange-500 text-white border border-white/20 backdrop-blur transition-all active:scale-95 cursor-pointer"
                                title="Next photo"
                            >
                                <IconChevronRight sz={12} />
                            </button>
                        </div>
                    )}

                    {/* Close Button */}
                    <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="absolute top-4 right-4 p-2 rounded-full bg-black/60 text-white hover:bg-black/90 backdrop-blur border border-white/20 transition-all hover:scale-110 cursor-pointer z-10"><IconX sz={18} /></button>

                    {/* Title Info with Brand Logo */}
                    <div className="absolute bottom-4 left-6 right-6 pointer-events-none">
                        <div className="flex justify-between items-end gap-3">
                            <div className="flex items-center gap-3 min-w-0">
                                <BrandLogo brand={v.brand} className="w-10 h-10 rounded-2xl bg-white p-1.5 shadow-lg border border-white/20 shrink-0" />
                                <div className="min-w-0">
                                    <div className="text-orange-400 text-xs font-bold uppercase tracking-wider mb-0.5 truncate">
                                        {v.brand} {v.model} • {v.type}
                                    </div>
                                    <div className="text-3xl font-black text-white drop-shadow-lg tracking-tight truncate">{v.plateNumber}</div>
                                </div>
                            </div>
                            <span className={`px-3.5 py-1.5 rounded-full text-xs font-bold uppercase backdrop-blur-md border border-[var(--border-color)] shadow-lg shrink-0 ${st.bg} ${st.text}`}>
                                {v.status}
                            </span>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        <div className="p-3.5 rounded-2xl bg-[var(--bg-tertiary)] backdrop-blur-md border border-[var(--border-color)] shadow-sm">
                            ...
                            <div className="text-lg font-bold text-[var(--text-primary)]">{parseInt(v.odometerKm || 0).toLocaleString()} <span className="text-xs text-[var(--text-muted)] font-normal">km</span></div>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-[var(--bg-tertiary)] backdrop-blur-md border border-[var(--border-color)] shadow-sm">
                            ...
                            <div className="text-lg font-bold text-[var(--text-primary)]">{v.currentSpeedKmh || 0} <span className="text-xs text-[var(--text-muted)] font-normal">km/h</span></div>
                        </div>
                        <div className="p-3.5 rounded-2xl bg-[var(--bg-tertiary)] backdrop-blur-md border border-[var(--border-color)] shadow-sm">
                            ...
                            <div className={`text-lg font-bold ${(v.healthScore ?? 98) > 80 ? 'text-emerald-500 dark:text-emerald-400' : (v.healthScore ?? 98) > 50 ? 'text-amber-500 dark:text-amber-400' : 'text-red-500 dark:text-red-400'}`}>{v.healthScore ?? 98}/100</div>
                        </div>
                    </div>

                    {/* Expressway RFID Section (Direct Centered Auto-Expanding Input) */}
                    <div>
                        <div className="flex justify-between items-center mb-2.5">
                            <h3 className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider flex items-center gap-1.5">
                                <IconTag sz={14} className="text-orange-500" /> Expressway RFID
                            </h3>
                            <div className="flex items-center gap-2">
                                {topupSuccess && (
                                    <span className="text-[11px] font-bold text-emerald-500 animate-fade-in flex items-center gap-1">
                                        <IconCheck sz={13} /> {topupSuccess}
                                    </span>
                                )}
                                <span className="text-[11px] font-semibold text-emerald-500 dark:text-emerald-400 flex items-center gap-1">
                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Active Toll Pass
                                </span>
                            </div>
                        </div>
                        
                        <div className={`p-4 rounded-2xl backdrop-blur-md border shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${rfidBal < 200 ? 'bg-red-500/10 border-red-500/30' : 'bg-[var(--bg-tertiary)] border-[var(--border-color)]'}`}>
                            <div>
                                <div className="text-[11px] font-semibold text-[var(--text-muted)] mb-0.5">Current Toll Balance</div>
                                <div className={`text-2xl font-black ${rfidBal < 200 ? 'text-red-500 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}`}>
                                    {formatPHP(rfidBal)}
                                </div>
                                {rfidBal < 200 && (
                                    <div className="text-[10px] text-red-500 font-bold mt-1 flex items-center gap-1">
                                        <IconAlertTriangle sz={11} /> Low Balance (Below ₱200)
                                    </div>
                                )}
                            </div>
                            
                            {/* Center-Aligned Auto-Expanding Input Box & Load Action */}
                            <div className="flex items-center gap-2">
                                <div className="flex items-center bg-[var(--bg-card)] border border-orange-500/40 rounded-xl px-2.5 py-1.5 focus-within:border-orange-500 shadow-sm focus-within:ring-2 focus-within:ring-orange-500/20 transition-all">
                                    <span className="text-xs font-black text-orange-500 select-none mr-1">₱</span>
                                    <input 
                                        type="text"
                                        inputMode="decimal"
                                        autoComplete="off"
                                        ...
                                        onKeyDown={e => {
                                            if (e.key === 'Enter' && reloadAmount && parseFloat(reloadAmount) > 0) {
                                                handleRfidTopup(reloadAmount);
                                            }
                                        }}
                                        style={{ width: `${Math.max(48, Math.min(140, (reloadAmount.length + 1) * 11))}px` }}
                                        className="bg-transparent border-none outline-none text-center font-black text-sm text-[var(--text-primary)] transition-all"
                                    />
                                </div>
                                <button 
                                    onClick={() => handleRfidTopup(reloadAmount)} 
                                    disabled={topupLoading || !reloadAmount || parseFloat(reloadAmount) <= 0}
                                    className="px-3.5 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 active:scale-95 text-white font-bold text-xs shadow-md shadow-orange-500/25 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50 disabled:pointer-events-none shrink-0"
                                >
                                    {topupLoading ? (
                    ...
                    {/* Legal Expiry & Documents Section */}
                    <div className="p-4 rounded-2xl bg-[var(--bg-tertiary)] backdrop-blur-md border border-[var(--border-color)] shadow-sm">
                        <div className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-3 flex items-center justify-between">
                            <span className="flex items-center gap-1.5"><IconFileText sz={14} /> Legal Expiry & Documents</span>
                            <IconFileText sz={14} className="text-[var(--text-muted)]" />
                        </div>
                        <div className="space-y-2.5">
                            <div className="p-3 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] flex items-center justify-between">
                                <div>
                                    <div className="text-xs font-bold text-[var(--text-primary)] flex items-center gap-1.5"><IconFileText sz={13} /> LTO Registration (OR/CR)</div>
                                    <div className="text-[10px] text-[var(--text-muted)] mt-0.5">Expiry: <span className="font-semibold text-[var(--text-secondary)]">{v.documents?.ltoRegistrationExpiry || '2027-02-25'}</span></div>
                                </div>
                                {(v.orCrUrl || v.or_cr_url) ? (
                                    <button onClick={() => onPreviewDoc(v.orCrUrl || v.or_cr_url, `LTO Registration (OR/CR) - ${v.plateNumber}`)} className="px-2.5 py-1 rounded-lg bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 font-bold text-[11px] border border-orange-500/20 flex items-center gap-1 cursor-pointer transition-colors">
                                        <IconFileText sz={12} /> View Document
                                    </button>
                                ) : (
                                    <span className="text-[10px] font-semibold text-[var(--text-muted)] px-2 py-0.5 rounded bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10">No Document Uploaded</span>
                                )}
                            </div>
                            <div className="p-3 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] flex items-center justify-between">
                                <div>
                                    <div className="text-xs font-bold text-[var(--text-primary)] flex items-center gap-1.5"><IconShield sz={13} /> CTPL Insurance Policy</div>
                                    <div className="text-[10px] text-[var(--text-muted)] mt-0.5">Expiry: <span className="font-semibold text-[var(--text-secondary)]">{v.documents?.insuranceExpiry || '2026-11-20'}</span></div>
                                </div>
                                {(v.insuranceUrl || v.insurance_url) ? (
                                    <button onClick={() => onPreviewDoc(v.insuranceUrl || v.insurance_url, `CTPL Insurance Policy - ${v.plateNumber}`)} className="px-2.5 py-1 rounded-lg bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 font-bold text-[11px] border border-orange-500/20 flex items-center gap-1 cursor-pointer transition-colors">
                                        <IconFileText sz={12} /> View Document
                                    </button>
                                ) : (
                                    <span className="text-[10px] font-semibold text-[var(--text-muted)] px-2 py-0.5 rounded bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10">No Document Uploaded</span>
                ...
                {/* Footer Controls */}
                <div className="p-4 px-6 border-t border-[var(--border-color)] bg-[var(--bg-card)] flex items-center justify-between gap-3">
                    <button onClick={() => { onClose(); onEdit(v); }} className="flex-1 py-2.5 px-4 rounded-xl bg-orange-500 hover:bg-orange-600 active:scale-95 text-white font-bold text-xs shadow-lg shadow-orange-500/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer">
                        <IconEdit sz={14} /> Edit Specs
                    </button>
                    <button onClick={() => { onDelete(v); }} className="py-2.5 px-4 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 font-bold text-xs transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer">
                        <IconTrash sz={14} /> Delete
                    </button>
                    <button onClick={() => { onShowQr(v); }} className="py-2.5 px-4 rounded-xl bg-[var(--bg-tertiary)] hover:bg-[var(--border-color)] text-[var(--text-primary)] border border-[var(--border-color)] font-bold text-xs transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer">
                        <IconQrCode sz={14} /> Handover QR
                    </button>
                </div>
            ...
            {isFullscreenImg && (
                <div 
                    className="fixed inset-0 bg-black/95 z-[999999] backdrop-blur-2xl flex flex-col items-center justify-between p-4 sm:p-6 animate-fade-in select-none"
                    onClick={(e) => { e.stopPropagation(); setIsFullscreenImg(false); }}
                >
                    {/* Lightbox Header Bar */}
                    <div className="w-full flex items-center justify-between z-10 max-w-5xl" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-3">
                            <BrandLogo brand={v.brand} className="w-9 h-9 rounded-xl bg-white p-1 shadow-md shrink-0" />
                            <div>
                                <div className="text-white font-black text-sm sm:text-base tracking-tight">{v.brand} {v.model} ({v.plateNumber})</div>
                                <div className="text-xs text-orange-400 font-bold">{photoIdx + 1} of {allPhotos.length || 1} Photos</div>
                            </div>
                        </div>
                        <button 
                            onClick={() => setIsFullscreenImg(false)} 
                            className="p-2.5 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur transition-all hover:scale-110 cursor-pointer"
                    ...
                    <div className="relative flex-1 w-full max-w-5xl flex items-center justify-center p-2 sm:p-4 my-2" onClick={e => e.stopPropagation()}>
                        {allPhotos.length > 1 && (
                            <button 
                                onClick={(e) => { e.stopPropagation(); setPhotoIdx(prev => (prev - 1 + allPhotos.length) % allPhotos.length); }} 
                                className="absolute left-2 sm:left-4 p-3 rounded-full bg-black/60 hover:bg-orange-500 text-white border border-white/20 shadow-2xl backdrop-blur transition-all active:scale-95 cursor-pointer z-10"
                                title="Previous Photo"
                            >
                                <IconChevronLeft sz={22} />
                            </button>
                        )}
                        
                        <img 
                            key={allPhotos[photoIdx % allPhotos.length] || resolveVehicleImgUrl(v.image)}
                            src={allPhotos[photoIdx % allPhotos.length] || resolveVehicleImgUrl(v.image)} 
                            alt={v.plateNumber} 
                            className="max-w-full max-h-[72vh] object-contain rounded-2xl shadow-2xl border border-white/10 animate-fade-in"
                        />

                        {allPhotos.length > 1 && (
                            <button 
                                onClick={(e) => { e.stopPropagation(); setPhotoIdx(prev => (prev + 1) % allPhotos.length); }} 
                                className="absolute right-2 sm:right-4 p-3 rounded-full bg-black/60 hover:bg-orange-500 text-white border border-white/20 shadow-2xl backdrop-blur transition-all active:scale-95 cursor-pointer z-10"
                                title="Next Photo"
                            >
                                <IconChevronRight sz={22} />
                            </button>
                        )}
                    </div>
                    ...
                    {allPhotos.length > 1 && (
                        <div className="flex items-center gap-2 p-2 rounded-2xl bg-white/5 border border-white/10 backdrop-blur z-10 overflow-x-auto max-w-full custom-scrollbar" onClick={e => e.stopPropagation()}>
                            {allPhotos.map((p, idx) => (
                                <div 
                                    key={idx} 
                                    onClick={() => setPhotoIdx(idx)} 
                                    className={`w-16 h-12 rounded-xl overflow-hidden border-2 cursor-pointer transition-all shrink-0 ${photoIdx === idx ? 'border-orange-500 scale-105 shadow-lg shadow-orange-500/40' : 'border-transparent opacity-50 hover:opacity-100'}`}
                                >
...
const FleetOverview = () => {
    ...
    // Derived Data (with Low RFID, Active Pass & Deep Field Filtering)
    const filteredVehicles = useMemo(() => {
        return vehicles.filter(v => {
            const st = (v.status || '').toLowerCase();
            const rfid = parseFloat(v.expressway_rfid_balance ?? v.expresswayRfidBalance ?? v.ExpresswayRfidBalance ?? v.rfid_balance_autosweep ?? v.rfidBalancePHP ?? 500);
            const mStat = filterStatus === 'all' ||
                (filterStatus === 'rented' ? (st === 'rented' || st === 'in-use') : 
                 filterStatus === 'low_rfid' ? (rfid < 200) : 
                 filterStatus === 'good_rfid' ? (rfid >= 200) : 
                 st === filterStatus);

            const term = (searchTerm || '').toLowerCase().trim();
            const rfidStr = String(Math.round(rfid));
            const mSearch = !term ||
                ...
                (term === 'low rfid' && rfid < 200) ||
                (term === 'rfid' && (rfid < 200 || term.includes('rfid'))) ||
                (term.includes('toll') || term.includes('pass') || term.includes('expressway') || term.includes('autosweep') || term.includes('easytrip')) ||
                (term.startsWith('p') && rfidStr.includes(term.replace(/[^0-9]/g, '')));
            ...
            <div className="flex-1 lg:max-w-[58%] h-full overflow-y-auto p-5 flex flex-col gap-5 custom-scrollbar">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div 
                            onClick={() => setFilterStatus(prev => prev === 'low_rfid' ? 'all' : 'low_rfid')}
                            className={`p-3 px-4 rounded-xl border flex items-center justify-between shadow-sm transition-all cursor-pointer select-none ${
                                filterStatus === 'low_rfid'
                                    ? 'bg-orange-500/30 border-orange-500 ring-2 ring-orange-500/40 shadow-orange-500/20'
                                    : 'bg-orange-500/15 border-orange-500/40 hover:bg-orange-500/25'
                            }`}
                            title="Click to filter vehicles with Low RFID Balance (< ₱200)"
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                                ...
                                {filterStatus === 'low_rfid' && (
                                    <span className="text-[9px] bg-orange-500 text-white px-1.5 py-0.5 rounded font-black tracking-wider uppercase">Filtered</span>
                                )}
                            </div>
                        </div>
                    </div>
